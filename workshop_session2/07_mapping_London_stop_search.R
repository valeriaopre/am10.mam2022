...
sep20 <- read_csv(here("data/stop-search/2020-09/2020-09-metropolitan-stop-and-search.csv")) %>% 
  clean_names() %>% 
...
sep20_offence_sf <-  st_as_sf(sep20_offence, 
                              coords=c('lng', 'lat'), 
...
ggplot() +
  ...
  geom_sf(
    data = sep20_offence_sf, aes(fill = object_of_search), 
  ...
  labs(title = "Locations of Stop&Search in London Sep 2020") +
...
ggplot() +
  ...
  labs(
    title = "Locations of Stop&Search in London Sep 2020"
  ) +
